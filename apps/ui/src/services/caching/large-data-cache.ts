...
interface CacheEntry<T = any> {
	...
	accessCount: number;
	lastAccessed: number;
	ttlMinutes?: number; // Custom TTL for this entry
}
...
export class LargeDataCache {
	...
	get<T>(key: string): T | null {
		...
		// Check if expired
		const now = Date.now();
		const ttlMinutes = entry.ttlMinutes ?? this.config.ttlMinutes;
...
export const largeDataCache = new LargeDataCache({
	maxCacheSize: 100_000, // 100k cache entries
	ttlMinutes: 15, // 15 minute TTL by default
});
