...
class IcebergReaderService {
	...
	async getGeographicData(
		type: "contract_locations" | "virtual_hq",
		filters?: Record<string, any>,
	): Promise<TableResponse> {
	...
	private async executeQuery<T = any>(
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Iceberg query failed: ${response.status} ${response.statusText}`,
				);
			}
...
export const icebergReader = new IcebergReaderService({
	baseUrl: import.meta.env.VITE_ICEBERG_API_URL || "http://localhost:8000/api",
	apiKey: import.meta.env.VITE_ICEBERG_API_KEY,
