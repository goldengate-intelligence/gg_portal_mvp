...
class LushaApiService {
  ...
  private cache = new Map<string, { data: CompanyContacts; timestamp: number }>();
   ...
   */
  async searchCompanyContacts(request: ContactSearchRequest): Promise<CompanyContacts> {
  ...
  async getContractorContacts(
    ...
    const searchRequest: ContactSearchRequest = {
      ...
      location: additionalInfo?.city || additionalInfo?.state ? {
   ...
   */
  filterContactsByDepartment(contacts: ContactPerson[], filters: Partial<DepartmentFilter>): ContactPerson[] {
    if (!Object.values(filters).some(Boolean)) return contacts;

    return contacts.filter(contact => {
      const dept = contact.department.toLowerCase();

      if (filters.sales && this.isDepartment(dept, ['sales', 'business development', 'account'])) return true;
      if (filters.marketing && this.isDepartment(dept, ['marketing', 'growth', 'communications'])) return true;
      if (filters.engineering && this.isDepartment(dept, ['engineering', 'development', 'technical', 'r&d'])) return true;
      if (filters.operations && this.isDepartment(dept, ['operations', 'manufacturing', 'production', 'supply'])) return true;
      if (filters.finance && this.isDepartment(dept, ['finance', 'accounting', 'controller'])) return true;
      if (filters.hr && this.isDepartment(dept, ['human resources', 'hr', 'people', 'talent'])) return true;
      if (filters.legal && this.isDepartment(dept, ['legal', 'compliance', 'regulatory'])) return true;
      if (filters.executive && this.isDepartment(dept, ['executive', 'leadership'])) return true;
   ...
   */
  filterContactsBySeniority(contacts: ContactPerson[], filters: Partial<SeniorityFilter>): ContactPerson[] {
    if (!Object.values(filters).some(Boolean)) return contacts;
  ...
  getContactStatistics(contacts: ContactPerson[]): {
    ...
    contacts.forEach(contact => {
      ...
      // Seniority stats
      bySeniority[contact.seniority] = (bySeniority[contact.seniority] || 0) + 1;
    ...
    return {
      ...
      withEmail,
      withPhone,
      avgConfidence: contacts.length > 0 ? totalConfidence / contacts.length : 0
    };
  }
  ...
  exportContactsToCSV(contacts: ContactPerson[]): string {
    const headers = ['Name', 'Title', 'Department', 'Seniority', 'Email', 'Phone', 'LinkedIn', 'Confidence'];
  ...
  clearCache(companyName?: string): void {
    if (companyName) {
      const keys = Array.from(this.cache.keys()).filter(key => key.includes(companyName));
   ...
   */
  private transformContactsResponse(data: any, request: ContactSearchRequest): CompanyContacts {
    const contacts: ContactPerson[] = (data.contacts || []).map((contact: any) => ({
      id: contact.id || `${contact.firstName}_${contact.lastName}_${Date.now()}`,
      ...
      confidence: parseFloat(contact.confidence || 0.8)
    }));
  ...
  private determineSeniority(title: string): ContactPerson['seniority'] {
    ...
    if (this.includesAny(normalizedTitle, ['ceo', 'cfo', 'cto', 'coo', 'chief', 'president', 'founder'])) return 'c-level';
    if (this.includesAny(normalizedTitle, ['vp', 'vice president'])) return 'vp';
    if (this.includesAny(normalizedTitle, ['director', 'head of'])) return 'director';
    if (this.includesAny(normalizedTitle, ['manager', 'lead', 'supervisor'])) return 'manager';
    if (this.includesAny(normalizedTitle, ['senior', 'sr.', 'principal'])) return 'senior';
    if (this.includesAny(normalizedTitle, ['junior', 'jr.', 'associate'])) return 'junior';
  ...
  private normalizeDepartment(dept: string): string {
    ...
    if (this.isDepartment(normalized, ['sales', 'business development'])) return 'Sales';
    if (this.isDepartment(normalized, ['marketing', 'growth'])) return 'Marketing';
    if (this.isDepartment(normalized, ['engineering', 'development', 'technical'])) return 'Engineering';
    if (this.isDepartment(normalized, ['operations', 'manufacturing'])) return 'Operations';
    if (this.isDepartment(normalized, ['finance', 'accounting'])) return 'Finance';
    ...
    if (this.isDepartment(normalized, ['executive', 'leadership'])) return 'Executive';
...
export const lushaApi = new LushaApiService({
  apiKey: process.env.VITE_LUSHA_API_KEY || '',
  baseUrl: process.env.VITE_LUSHA_API_URL || 'https://api.lusha.com/v2',
