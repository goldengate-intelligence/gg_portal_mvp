...
export function exportContractors(
	...
	const exportData = contractors.map((c) => ({
		...
		"Last Verified": c.lastVerified
			? new Date(c.lastVerified).toLocaleDateString()
			: "",
	}));
...
export function exportOpportunities(
	...
	const exportData = opportunities.map((o) => ({
		...
		"Posted Date": o.postedDate
			? new Date(o.postedDate).toLocaleDateString()
			: "",
		"Response Deadline": o.responseDeadline
