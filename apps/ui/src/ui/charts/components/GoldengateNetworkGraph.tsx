...
export function GoldengateNetworkGraph({
    ...
    */
	...
	useEffect(() => {
		...
		nodesRef.current = initializeNodePositions(
			nodes,
			edges,
			displayWidth,
			displayHeight,
		...
		console.log(
			"Initialized nodes with positions:",
		...
		const handleMouseMove = (e: MouseEvent) => {
			...
			if (isDragging && draggedNode) {
				...
				const clickedNode = nodesRef.current.find((node) => {
					...
					const radius =
						Math.max(20, Math.min(40, Math.sqrt(node.value / 1000000) * 3)) *
						sizeFactor;
			...
			edges.forEach((edge) => {
				...
				if (other) {
					...
					const sourceRadius =
						15 +
						(60 - 15) *
							(Math.log10(Math.max(1, node.value / 1000000) + 1) /
								Math.log10(10000 + 1));
					const targetRadius =
						15 +
						(60 - 15) *
							(Math.log10(Math.max(1, other.value / 1000000) + 1) /
								Math.log10(10000 + 1));
	...
	function drawAnimatedEdge(
		...
		ctx.beginPath();
		ctx.moveTo(
			arrowX + arrowOffset * Math.cos(angle),
			arrowY + arrowOffset * Math.sin(angle),
		); // Tip at edge
		ctx.lineTo(
    ...
    */
		...
		const label =
			node.label.length > maxLength
				? `${node.label.substring(0, maxLength - 2)}..`
				: node.label;
