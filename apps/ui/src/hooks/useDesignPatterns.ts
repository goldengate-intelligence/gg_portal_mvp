...
export const useDesignPatterns = () => {
	...
	const PanelWrapper = {
		hudCard:
			"border-gray-700/30 rounded-xl overflow-hidden shadow-2xl hover:border-gray-600/20 transition-all duration-500 group",
	...
	const Typography = {
		...
		// Status badges
		statusBadge:
			"px-1.5 py-0.5 rounded-full uppercase tracking-wider font-sans font-normal",
	...
	const InteractiveStates = {
		...
		tooltip:
			"bg-black border border-gray-700 rounded-md px-3 py-2 text-xs text-gray-300",
		tooltipWrapper:
			"absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50",
	...
	const AccentPatterns = {
		...
		scanLine:
			"absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-gray-300/10 to-transparent animate-scan",
		gradientOverlay:
			"absolute inset-0 bg-gradient-to-br from-gray-700/3 to-gray-900/2",
	};
