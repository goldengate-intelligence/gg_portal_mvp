...
export function useContractorLists() {
	...
	const updateList = useCallback(
		async (listId: string, updates: Partial<ContractorList>) => {
			try {
				...
				const updatedList = await apiClient.updateContractorList(
					listId,
					cleanedUpdates,
				);
				setLists((prev) =>
...
export function useContractorFavorites() {
	...
	const toggleFavorite = useCallback(async (contractorProfileId: string) => {
		try {
			...
			const result =
				await apiClient.toggleContractorFavorite(contractorProfileId);
	...
	const checkFavorites = useCallback(async (contractorProfileIds: string[]) => {
		try {
			const results =
				await apiClient.checkContractorFavorites(contractorProfileIds);
	...
	return {
		favorites,
		isLoading,
		isFavorite: (contractorProfileId: string) =>
			favorites.has(contractorProfileId),
		toggleFavorite,
		checkFavorites,
...
export function useContractorListItems(listId: string | null) {
	const [items, setItems] = useState<
		Array<ContractorListItem & { contractor: any }>
	>([]);
	...
	const fetchListItems = useCallback(async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to fetch list items",
			);
			setItems([]);
			setList(null);
		} finally {
	...
	const removeFromList = useCallback(
		async (contractorProfileId: string) => {
			if (!listId) return;
			...
			try {
				...
				setItems((prev) =>
					prev.filter(
						(item) => item.contractorProfileId !== contractorProfileId,
					),
				);
			} catch (err) {
