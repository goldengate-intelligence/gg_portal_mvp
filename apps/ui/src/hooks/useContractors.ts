...
export function useContractors(
	...
	const [pagination, setPagination] =
		useState<UseContractorsResult["pagination"]>(null);
	const [aggregations, setAggregations] =
		useState<UseContractorsResult["aggregations"]>(null);
	...
	const fetchContractors = useCallback(async () => {
		...
		try {
			...
			if (filters.lifecycleStages?.length)
				params.lifecycle = filters.lifecycleStages.join(",");
			if (filters.sizeTiers?.length) params.size = filters.sizeTiers.join(",");
			if (filters.minObligated !== undefined)
				params.minObligated = filters.minObligated;
			if (filters.maxObligated !== undefined)
				params.maxObligated = filters.maxObligated;
			if (filters.minContracts !== undefined)
				params.minContracts = filters.minContracts;
			if (filters.maxContracts !== undefined)
				params.maxContracts = filters.maxContracts;
...
export function useTopContractors(
	...
	const fetchTopContractors = useCallback(async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to fetch top contractors",
			);
			setData([]);
		} finally {
			setIsLoading(false);
