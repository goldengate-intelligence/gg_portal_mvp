...
import {
	...
	type ContractorProfileQueryResult,
	apiClient,
...
export function useContractorProfiles(
	options: UseContractorProfilesOptions = {},
): UseContractorProfilesResult {
	const {
		page = 1,
		limit = 24,
	...
	const [pagination, setPagination] =
		useState<UseContractorProfilesResult["pagination"]>(null);
	const [aggregations, setAggregations] =
		useState<UseContractorProfilesResult["aggregations"]>(null);
	...
	const fetchContractorProfiles = useCallback(async () => {
		...
		try {
			...
			if (filters.sizeTiers?.length)
				params.sizeTiers = filters.sizeTiers.join(",");
			if (filters.lifecycleStages?.length)
				params.lifecycleStages = filters.lifecycleStages.join(",");
			if (filters.minObligated !== undefined)
				params.minObligated = filters.minObligated.toString();
			if (filters.maxObligated !== undefined)
				params.maxObligated = filters.maxObligated.toString();
			...
			setError(
				err instanceof Error
					? err.message
					: "Failed to fetch contractor profiles",
			);
			setData([]);
			setPagination(null);
	...
	const fetchTopProfiles = useCallback(async () => {
		...
		try {
			...
			setError(
				err instanceof Error
					? err.message
					: "Failed to fetch top contractor profiles",
			);
			setData([]);
		} finally {
			setIsLoading(false);
