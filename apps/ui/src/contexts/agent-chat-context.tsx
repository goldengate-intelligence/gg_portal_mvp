...
const AgentChatContext = createContext<AgentChatContextValue | undefined>(
	undefined,
);

export function AgentChatProvider({ children }: { children: React.ReactNode }) {
	const [isOpen, setIsOpen] = useState(false);
	const [entityContext, setEntityContext] =
		useState<AgentChatContextValue["entityContext"]>(null);
	const [customMessage, setCustomMessage] = useState<string | null>(null);

	const open = useCallback(
		(context: { entityId: string; entityName: string; entityType: string }) => {
