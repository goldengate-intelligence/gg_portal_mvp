	...
	for (const profile of Object.values(INDUSTRY_PROFILES)) {
		...
		if (
			profile.keywords.some(
				(keyword) => company.includes(keyword) || naics.includes(keyword),
			)
		) {
			return profile;
		}
...
export const getAISummaryPrompts = (
	companyName: string,
	naicsDescription: string,
): string[] => {
