...
class PortfolioPinningLogic {
  ...
  savePinningState(state: PinningState): void {
    try {
      ...
      localStorage.setItem(PortfolioPinningLogic.STORAGE_KEY, JSON.stringify(toStore));
  ...
  togglePin(
    ...
    assetId: string,
    assetType: 'asset' | 'group' = 'asset'
  ): { success: boolean; newState: PinningState; message: string; isPinned: boolean } {
  ...
  reorderPinnedAssets(
    ...
    const invalidItems = newOrder.filter(id => !currentState.pinnedAssets.has(id));
  ...
  getPinningStatistics(
    ...
    const pinnedAssetsList = assets.filter(asset => pinnedAssets.has(asset.id));
    ...
    const pinnedPercentage = totalPortfolioValue > 0
  ...
  getPinningRecommendations(
    ...
    const unpinnedAssets = assets.filter(asset => !currentlyPinned.has(asset.id));
  ...
  private getAssetScore(asset: PortfolioAsset | GroupAsset): number {
    if ('type' in asset) {
      ...
      const memberScores = asset.memberAssets.map(member => member.performanceScore);
      return memberScores.reduce((sum, score) => sum + score, 0) / memberScores.length;
    }
