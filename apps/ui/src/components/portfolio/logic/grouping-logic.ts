...
class PortfolioGroupingLogic {
	...
	createGroup(
		...
		try {
			...
			const newGroup = portfolioDataService.createAssetGroup(
				assetsToGroup,
				groupName,
			);
		...
		try {
			const updatedAssets = assets.map((asset) => {
				if (asset.id === groupId && "type" in asset) {
					...
					return portfolioDataService.updateAssetGroup(group, {
						groupName: newName,
					});
				}
				return asset;
	 ...
	 */
	validateGrouping(operation: GroupingOperation): {
		valid: boolean;
		message: string;
	} {
		switch (operation.type) {
			case "create":
				...
				if (operation.assetIds.length < 2) {
					return {
						valid: false,
						message: "At least 2 assets required for grouping",
					};
				}
				break;
			...
			case "add_to_existing":
				...
				if (operation.assetIds.length === 0) {
					return {
						valid: false,
						message: "No assets specified to add to group",
					};
				}
				break;
	...
	private extractIndustryFromNaics(naicsDescription: string): string {
		...
		if (desc.includes("manufacturing") || desc.includes("fabricat"))
