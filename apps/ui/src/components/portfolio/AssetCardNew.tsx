...
export function AssetCardNew({
	...
	const getCardTheme = (name: string) => {
		...
		if (name.includes("GreenPoint"))
			return {
				accent: "#059669",
				progress: "from-emerald-600 to-green-500",
				fill: "70%",
			};
		if (name.includes("QuantumShield"))
			return {
				accent: "#DC2626",
				progress: "from-red-600 to-rose-500",
				fill: "55%",
			};
		if (name.includes("NextGen"))
			return {
				accent: "#0891B2",
	...
	const [contractorMetrics, setContractorMetrics] = useState<any>(null);
	const [isLoadingMetrics, setIsLoadingMetrics] = useState(false);

	useEffect(() => {
		const loadContractorMetrics = async () => {
			if (!uei || isLoadingMetrics) return;

			setIsLoadingMetrics(true);
			try {
				let metrics = await contractorMetricsService.getContractorMetrics(uei);
				if (!metrics) {
					metrics =
						await contractorMetricsService.getContractorMetricsByName(
							companyName,
						);
				}
				if (!metrics) {
					metrics = contractorMetricsService.getDefaultMetrics(
						uei,
						companyName,
					);
				}
				...
				console.warn("Failed to load contractor metrics:", error);
				setContractorMetrics(
					contractorMetricsService.getDefaultMetrics(uei, companyName),
				);
			} finally {
	...
	}, [uei, companyName, isLoadingMetrics]);
	...
	useEffect(() => {
		if (uei && companyName) {
			console.log(`[AssetCard] Starting logo fetch for ${companyName} (${uei})`);
			setLogoLoading(true);
			setLogoError(null);
			setContractorLogo(null); // Clear any previous logo

			contractorLogoService
				.getContractorLogo(uei, companyName)
				.then((response) => {
					console.log(`[AssetCard] Logo response for ${companyName}:`, response);
					if (response.logoUrl) {
						console.log(`[AssetCard] Setting logo URL for ${companyName}: ${response.logoUrl}`);
						setContractorLogo(response.logoUrl);
					} else {
						console.log(`[AssetCard] No logo URL returned for ${companyName}, will show initials`);
						setContractorLogo(null);
					}
					setLogoLoading(false);
				})
				.catch((error) => {
					console.warn(`[AssetCard] Logo failed for ${companyName}:`, error);
					setLogoError(error.message || 'Unknown error');
					setContractorLogo(null);
					setLogoLoading(false);
				});
		} else {
			console.log(`[AssetCard] Missing UEI or companyName for logo fetch: ${uei}, ${companyName}`);
			setLogoLoading(false);
			setContractorLogo(null);
		}
	}, [uei, companyName]);
	...
	const metrics =
		aggregatedMetrics || getContractorFinancialMetrics(uei, companyName);

	// Check if this is a defense contractor by using loaded contractor details
	const isDefenseContractor =
		contractorMetrics?.primaryAgency?.toLowerCase().includes("defense") ||
		...
		<>
			<div
				className={`border ${isExpanded ? "rounded-xl" : "rounded-xl"} cursor-pointer overflow-visible relative ${
					isModalOpen
						? ""
						: "transform hover:scale-[1.02] hover:shadow-xl transition-transform duration-300 ease-in-out"
				} ${
					isDraggedOver
			...
			>
				...
				>
					<div
						className={`absolute inset-0.5 bg-gradient-to-br from-gray-900/90 via-gray-800/70 to-gray-900/50 transition-all duration-300 ${
							isExpanded ? "rounded-t-xl" : "rounded-xl"
						}`}
					/>
					<div className="relative z-10 p-4 h-full">
						...
						<div className="flex items-start justify-between h-full">
							...
							<div className="w-20 h-20 bg-gradient-to-br from-[#D2AC38]/10 via-transparent to-[#D2AC38]/5 rounded-lg border-2 border-[#D2AC38]/40 flex items-center justify-center flex-shrink-0 relative overflow-hidden mr-4 transition-all duration-300 hover:border-[#D2AC38]/60 hover:from-[#D2AC38]/15 hover:to-[#D2AC38]/8">
								{logoLoading ? (
									<div className="text-[#D2AC38]/60 text-xs animate-pulse">Loading...</div>
								) : contractorLogo ? (
									<>
										<img
											src={contractorLogo}
											...
											onLoad={(e) => {
												console.log(`Logo loaded successfully for ${companyName}: ${contractorLogo}`);
												// Hide initials when image loads successfully
												const initialsElement = e.currentTarget.parentElement?.querySelector('.logo-initials') as HTMLElement;
												if (initialsElement) {
													initialsElement.style.display = "none";
												}
											}}
											onError={(e) => {
												console.warn(`Image failed to load for ${companyName}: ${contractorLogo}`);
												// Hide the image and show initials as fallback
										...
										/>
										{/* Company initials - hidden by default when logo is available, shown as fallback on error */}
										<span
											className="logo-initials text-[#D2AC38] text-lg font-bold uppercase absolute inset-0 flex items-center justify-center"
											style={{
												fontFamily: "Michroma, sans-serif",
												display: "none",
											}}
											title={logoError ? `Logo error: ${logoError}` : `Fallback for ${companyName}`}
										>
											{initials}
										</span>
									</>
									...
									<span
										className="logo-initials text-[#D2AC38] text-lg font-bold uppercase"
										style={{
											fontFamily: "Michroma, sans-serif",
										}}
										title={logoError ? `Logo error: ${logoError}` : `${companyName} initials`}
									>
										{initials}
									</span>
								)}
													...
													{(() => {
														...
														const getGroupBusinessSummary = () => {
															...
															groupMembers.forEach((member) => {
																const description =
																	member?.naicsDescription?.toLowerCase() || "";
																if (
																	description.includes("aircraft") ||
																	description.includes("aerospace")
																)
																	industries.add("Aerospace");
																if (
																	description.includes("manufacturing") ||
																	description.includes("fabricat")
																)
															...
															if (industries.size === 0)
																return "Diversified Operations";
															if (industries.size === 1)
																return `${Array.from(industries)[0]} Consortium`;
															if (industries.size === 2)
																return `${Array.from(industries).slice(0, 2).join(" & ")} Alliance`;
											...
											<div className="flex items-center justify-between">
												<div className="font-normal text-[#F97316]/90 text-xs">
													{(() => {
														...
														const awards =
															contractorMetrics?.contractCount ||
															Math.floor(Math.random() * 5) + 2; // 2-6 awards
												...
												<div
													className={`px-2 py-0.5 rounded-full text-xs transition-all duration-300 hover:scale-105 ${isDefenseContractor ? "text-red-400 bg-red-400/10 hover:bg-red-400/20 hover:text-red-300" : "text-teal-400 bg-teal-400/10 hover:bg-teal-400/20 hover:text-teal-300"}`}
												>
				...
				<div className="absolute top-4 right-4 z-30 flex flex-col items-end gap-2">
					...
					<div
						className={`flex items-center px-3 py-1 bg-gray-600/20 border border-gray-600/40 rounded-full gap-2 transition-all duration-200 ${
					...
					>
						...
						>
							<div
								className={`p-0.5 rounded cursor-pointer ${
									isModalOpen
										? ""
										: "hover:bg-[#D2AC38]/30 hover:scale-110 transition-all duration-300"
						...
						<div
							className={`p-0.5 rounded cursor-pointer relative ${
								isModalOpen
									? ""
									: "hover:bg-purple-500/30 hover:scale-110 transition-all duration-300"
							}`}
							onMouseEnter={(e) => {
								e.stopPropagation();
						...
						<div
							className={`p-0.5 rounded cursor-pointer relative ${
								isModalOpen
									? ""
									: "hover:bg-cyan-500/30 hover:scale-110 transition-all duration-300"
							}`}
							onMouseEnter={(e) => {
								e.stopPropagation();
						...
						<div
							className={`p-0.5 rounded cursor-pointer relative ${
								isModalOpen
									? ""
									: "hover:bg-teal-500/30 hover:scale-110 transition-all duration-300"
							}`}
							onMouseEnter={(e) => {
								e.stopPropagation();
						...
						<div
							className={`p-0.5 rounded cursor-pointer relative ${
								isModalOpen
									? ""
									: "hover:bg-orange-500/30 hover:scale-110 transition-all duration-300"
						...
						>
							<Pin
								className={`w-4 h-4 hover:text-orange-300 transition-colors ${
