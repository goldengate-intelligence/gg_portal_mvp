...
export function KnowledgeBaseModal({
  ...
  const getFileIcon = (type: string, name: string) => {
    ...
    if (lowerType.startsWith('image/')) return <Image className="w-4 h-4 text-green-400" />;
    if (lowerType.startsWith('video/')) return <Video className="w-4 h-4 text-purple-400" />;
    if (lowerType.startsWith('audio/')) return <Music className="w-4 h-4 text-pink-400" />;
    if (lowerType.includes('pdf') || lowerType.includes('document') || lowerType.includes('text') || extension === 'md') return <FileText className="w-4 h-4 text-red-400" />;
    if (lowerType.includes('spreadsheet') || extension === 'xlsx' || extension === 'csv') return <FileSpreadsheet className="w-4 h-4 text-emerald-400" />;
    if (lowerType.includes('zip') || lowerType.includes('rar') || extension === '7z') return <Archive className="w-4 h-4 text-orange-400" />;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Search Bar */}
        <div className="border-b border-gray-700/30 px-6 py-3" style={{ backgroundColor: CONTRACTOR_DETAIL_COLORS.panelColor }}>
          ...
          {filteredFiles.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center p-6">
              ...
              {searchQuery ? (
                ...
                <>
                  <h3 className="text-lg font-medium text-gray-300 mb-2">Knowledge base is empty</h3>
                  <p className="text-sm text-gray-500">Upload files using the attachment icon to build your knowledge base</p>
                </>
              )}
            </div>
          ) : (
