...
export function AssetsTab({ assets, setAssets }: AssetsTabProps) {
  ...
  const parseFinancialValue = (value: string): number => {
    ...
    const multiplier = value.includes('B') ? 1000000000 : value.includes('M') ? 1000000 : 1;
  ...
  const handleGroupDrop = (draggedAssetId: string, targetAssetId: string) => {
    ...
    const targetAsset = assets.find(asset => asset.id === targetAssetId || asset.uei === targetAssetId);
    const draggedAsset = assets.find(asset => asset.id === draggedAssetId || asset.uei === draggedAssetId);
    ...
    if ('type' in targetAsset && targetAsset.type === 'group') {
      ...
      console.log('Current pinned assets before group modification:', Array.from(pinnedAssets));

      // Check if dragged asset was pinned
      const draggedWasPinned = pinnedAssets.has(draggedAssetId) || pinnedAssets.has(draggedAsset.uei) ||
                              ('type' in draggedAsset && draggedAsset.type === 'group' && pinnedAssets.has(draggedAsset.id));

      console.log('Dragged asset pin status:', draggedWasPinned);

      // Add to existing group
      const updatedAssets = assets.map(asset => {
        if ((asset.id === targetAssetId || asset.uei === targetAssetId) && 'type' in asset && asset.type === 'group') {
          ...
          const totalActiveAwards = newMemberAssets.reduce((sum, memberAsset) =>
            sum + parseFinancialValue(memberAsset.activeAwards.value), 0
          );
        ...
        return asset;
      }).filter(asset => asset.id !== draggedAssetId && asset.uei !== draggedAssetId); // Remove the dragged asset
      ...
      updatedPinOrder = updatedPinOrder.filter(id =>
        id !== draggedAssetId && id !== draggedAsset.uei &&
        !('type' in draggedAsset && draggedAsset.type === 'group' && id === draggedAsset.id)
      );
      ...
      if (draggedWasPinned) {
        ...
        console.log('Ensuring target group becomes most recently pinned due to dragged asset pin status');
      }

      setPinnedAssets(updatedPinnedAssets);
      setPinOrder(updatedPinOrder);
      console.log('Updated pinned assets after group modification:', Array.from(updatedPinnedAssets));
      console.log('Updated pin order after group modification:', updatedPinOrder);
  ...
  const handleGroupConfirm = (groupName: string) => {
    ...
    const targetAsset = assets.find(asset => asset.id === targetAssetId) as Asset;
    const draggedAsset = assets.find(asset => asset.id === draggedAssetId) as Asset;

    if (!targetAsset || !draggedAsset) return;

    const memberAssets = [targetAsset, draggedAsset];
    const totalActiveAwards = memberAssets.reduce((sum, asset) =>
      sum + parseFinancialValue(asset.activeAwards.value), 0
    );
    ...
    console.log('Creating group from assets:', { targetAssetId, draggedAssetId });
    console.log('Current pinned assets before group creation:', Array.from(pinnedAssets));

    // Check if any of the original assets were pinned
    const targetWasPinned = pinnedAssets.has(targetAssetId) || pinnedAssets.has(targetAsset.uei);
    const draggedWasPinned = pinnedAssets.has(draggedAssetId) || pinnedAssets.has(draggedAsset.uei);
    ...
    updatedPinOrder = updatedPinOrder.filter(id =>
      id !== targetAssetId && id !== targetAsset.uei &&
      id !== draggedAssetId && id !== draggedAsset.uei
    );
  ...
  const getDefaultGroupName = () => {
    if (!pendingGroup) return '';
    const targetAsset = assets.find(asset => asset.id === pendingGroup.targetAssetId);
    return targetAsset ? `${targetAsset.companyName.split(' ')[0]} Group` : 'New Group';
  };
  ...
  const handleRemoveGroupMember = (memberUei: string) => {
    ...
    const removedMember = selectedGroup.memberAssets.find(member => member.uei === memberUei);
    if (!removedMember) return;

    const updatedGroup = {
      ...selectedGroup,
      memberAssets: selectedGroup.memberAssets.filter(member => member.uei !== memberUei)
    };
    ...
    if (updatedGroup.memberAssets.length === 1) {
      ...
      const totalActiveAwards = updatedGroup.memberAssets.reduce((sum, member) =>
        sum + parseFinancialValue(member.activeAwards.value), 0
      );
      ...
      const updatedAssets = assets
        .map(asset => asset.id === selectedGroup.id ? recalculatedGroup : asset)
  ...
  const handlePin = (uei: string) => {
    ...
    console.log('New pinned assets after operation:', Array.from(newPinnedAssets));
    ...
    setAssets(prev => {
      ...
      console.log('Updated assets with pin status:', updated.map(a => ({
        id: a.id,
        uei: a.uei,
  ...
  const sortedAssets = [...assets].sort((a, b) => {
    ...
    console.log('Sorting comparison:', {
      a: { name: a.companyName, identifier: aIdentifier, isPinned: aIsPinned, pinIndex: aPinIndex },
      b: { name: b.companyName, identifier: bIdentifier, isPinned: bIsPinned, pinIndex: bPinIndex }
    });
  ...
  console.log('All assets before sorting:', assets.map(a => ({
    name: a.companyName,
    id: a.id,
    uei: a.uei,
    identifier: getAssetIdentifier(a),
  ...
  console.log('Sorted assets order:', sortedAssets.map(a => ({
    name: a.companyName,
    identifier: getAssetIdentifier(a),
    isPinned: pinnedAssets.has(getAssetIdentifier(a)),
    pinIndex: pinOrder.indexOf(getAssetIdentifier(a)),
                    ...
                    {sortedAssets.map((asset, index) => (
                      <div key={asset.id} className="relative group transition-all duration-300 ease-in-out" style={{ animationDelay: `${index * 50}ms` }}>
                        ...
                        <AssetCardNew
                          ...
                          isGrouped={'type' in asset && asset.type === 'group'}
                          groupMembers={'type' in asset && asset.type === 'group' ? asset.memberAssets.map(m => m.id) : []}
