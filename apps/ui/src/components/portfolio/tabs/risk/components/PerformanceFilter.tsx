...
export function PerformanceFilter({
	...
	React.useEffect(() => {
		...
		if (filterMode === "value" && currentFeature === "composite_score") {
			...
			const firstValueFeature = Object.entries(featureOptions).find(
				([key, option]) =>
					option.category === "performance" && key !== "composite_score",
			);
		...
		filterMode,
		tempSettings.performance.feature,
		setTempSettings,
		featureOptions,
	]);
	...
	React.useEffect(() => {
		...
		if (filterMode === "value" && selectedFeature.unit !== "pts") {
			...
			if (
				...
				console.log(
					"Converted to value mode ranges for",
	...
	const getSelectedFeature = () => {
		const feature =
			featureOptions[tempSettings.performance.feature || "composite_score"];
	...
	const rangeConfig = React.useMemo(() => {
		const selectedFeature = getSelectedFeature();
		console.log(
			"getRangeConfig called - filterMode:",
			filterMode,
			"selectedFeature:",
			selectedFeature,
		);
		...
		<div className="border border-gray-700/30 rounded-xl p-6 bg-cyan-900/40">
			...
			<div className="mb-6">
				...
				<p className="text-sm text-gray-400 leading-relaxed mb-4">
					{tempSettings.performance.description}
				</p>
			</div>
						...
						>
							{Object.entries(featureOptions)
								.filter(([key, option]) => {
									...
									if (filterMode === "value" && key === "composite_score")
										return false;
									return true;
								})
								.map(([key, option]) => (
				...
				<div className="space-y-4">
					<div className="flex items-center space-x-3">
						...
						<input
							...
							onChange={(e) => {
								const newOptimalValue = Number.parseInt(e.target.value);
								setTempSettings((prev) => ({
									...prev,
									performance: {
										...prev.performance,
										optimal: {
											...prev.performance.optimal,
											min: newOptimalValue,
										},
										caution: {
											...prev.performance.caution,
											min: Math.min(
												prev.performance.caution.min,
												newOptimalValue - 1,
											),
											max: Math.max(
												prev.performance.caution.max,
												newOptimalValue,
											),
										},
										critical: {
						...
						<span className="text-sm text-gray-400 w-12 text-right">
							{rangeConfig.formatValue(tempSettings.performance.optimal.min)}
						</span>
					</div>
					...
					<div className="flex items-center space-x-3">
						...
						<input
							...
							onChange={(e) => {
								const newCautionValue = Number.parseInt(e.target.value);
								setTempSettings((prev) => ({
									...prev,
									performance: {
										...prev.performance,
						...
						/>
						<span className="text-sm text-gray-400 w-12 text-right">
							{rangeConfig.formatValue(tempSettings.performance.caution.min)}
						</span>
					</div>
						...
						<div className="text-xs space-y-1">
							<div className="flex justify-between text-green-400">
								...
								<span className="text-right">
									{rangeConfig.formatValue(
										tempSettings.performance.optimal.min,
									)}
									+
								</span>
							</div>
							<div className="flex justify-between text-yellow-400">
								...
								<span className="text-right">
									{rangeConfig.formatValue(
										tempSettings.performance.caution.min,
									)}
									-
									{rangeConfig.formatValue(
										tempSettings.performance.optimal.min - 1,
									)}
								</span>
							</div>
							<div className="flex justify-between text-red-400">
								...
								<span className="text-right">
									{rangeConfig.formatValue(0)}-
									{rangeConfig.formatValue(
										tempSettings.performance.critical.max,
									)}
								</span>
							</div>
						</div>
			...
			<div className="mt-4 pt-4 border-t border-gray-700/30">
				...
				>
					...
					{isCreatingFilter
						? "Save New Performance Filter"
						: "Save Performance Filter"}
				</Button>
			</div>
		</div>
	);
