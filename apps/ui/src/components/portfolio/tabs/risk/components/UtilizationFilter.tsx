		...
		if (existingFilterIndex !== -1) {
			...
			const updatedFilter: SavedUtilizationFilter = {
				...
				name:
					...
					`${entityId === "all_entities" ? "All Entities" : portfolioAssets.find((a) => a.uei === entityId)?.companyName || entityId} ${tempSettings.utilization.name || "Utilization"}`,
				config: { ...tempSettings.utilization },
				createdAt: new Date(),
			};
		...
		<div className="border border-gray-700/30 rounded-xl p-6 bg-indigo-900/40">
			...
			<div className="mb-6">
				...
				<p className="text-sm text-gray-400 leading-relaxed mb-4">
					{tempSettings.utilization.description}
				</p>
			</div>
			...
			>
				...
				{monitoringType === "central_band" ? (
					<div className="space-y-4">
						...
						<div>
							<div className="flex items-center space-x-3 mb-2">
								...
								<span className="text-xs text-gray-500">
									{tempSettings.utilization.optimal.min}% -{" "}
									{tempSettings.utilization.optimal.max}%
								</span>
							</div>
							<div className="grid grid-cols-2 gap-3">
								<div className="flex items-center space-x-2">
									...
									<input
										...
										onChange={(e) => {
											const newOptimalMin = Number.parseInt(e.target.value);
											setTempSettings((prev) => ({
												...prev,
												utilization: {
													...prev.utilization,
													optimal: {
														...prev.utilization.optimal,
														min: newOptimalMin,
													},
													caution: {
														...prev.utilization.caution,
														min: Math.min(
															prev.utilization.caution.min,
															newOptimalMin - 5,
														),
														max: Math.max(
															prev.utilization.caution.max,
															newOptimalMin,
														),
													},
												},
								...
								<div className="flex items-center space-x-2">
									...
									<input
										...
										onChange={(e) => {
											const newOptimalMax = Number.parseInt(e.target.value);
											setTempSettings((prev) => ({
												...prev,
												utilization: {
													...prev.utilization,
						...
						<div>
							<div className="flex items-center space-x-3 mb-2">
								...
								<span className="text-xs text-gray-500">
									{tempSettings.utilization.caution.min}% -{" "}
									{tempSettings.utilization.caution.max}%
								</span>
							</div>
							<div className="grid grid-cols-2 gap-3">
								<div className="flex items-center space-x-2">
							...
							<div className="text-xs space-y-1">
								<div className="flex justify-between text-red-400">
									...
									<span className="text-right">
										0%-{tempSettings.utilization.caution.min - 1}%
									</span>
								</div>
								<div className="flex justify-between text-yellow-400">
									...
									<span className="text-right">
										{tempSettings.utilization.caution.min}%-
										{tempSettings.utilization.optimal.min - 1}%
									</span>
								</div>
								...
								<div className="flex justify-between text-yellow-400">
									...
									<span className="text-right">
										{tempSettings.utilization.optimal.max + 1}%-
										{tempSettings.utilization.caution.max}%
									</span>
								</div>
								<div className="flex justify-between text-red-400">
									...
									<span className="text-right">
										{tempSettings.utilization.caution.max + 1}%+
									</span>
								</div>
							</div>
						</div>
					...
					<div className="space-y-4">
						...
						<div className="flex items-center space-x-3">
							<span className="text-xs text-yellow-400 w-20">
								CAUTION {">"}
							</span>
							<input
								type="range"
						...
						<div className="border-t border-gray-600/30 pt-3 mt-4">
							<div className="text-xs space-y-1">
								...
								<div className="flex justify-between text-red-400">
									...
									<span className="text-right">
										0%-{tempSettings.utilization.critical.max}%
									</span>
								</div>
							</div>
						</div>
			...
			<div className="mt-4 pt-4 border-t border-gray-700/30">
				...
				>
					...
					{isCreatingFilter
						? "Save New Utilization Filter"
						: "Save Utilization Filter"}
				</Button>
			</div>
		</div>
	);
