...
export function ActiveMonitorsList({ activeMonitors, onRemoveMonitor }: ActiveMonitorsListProps) {
              ...
              <div className="pt-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {activeMonitors.map((monitor) => (
                    ...
                    >
                      {/* Monitor Type Badge */}
                      <div className={`text-xs px-2 py-1 rounded-md font-medium mb-2 inline-block ${
                        monitor.type === 'activity'
                      ...
                      <div className="text-xs text-gray-500 mb-2">
                        ...
                        {monitor.type === 'performance' && monitor.config.type === 'range' && (
                          <div>Range: {monitor.config.critical?.max}-{monitor.config.optimal?.min}</div>
                        )}
                        {monitor.type === 'utilization' && monitor.config.type === 'range' && (
                          <div>Range: {monitor.config.caution?.max ?? 85}%+</div>
                        )}
                      </div>
                ...
                <div className="mt-4 text-center">
                  <p className="text-xs text-gray-500">
                    {activeMonitors.length} active monitor{activeMonitors.length !== 1 ? 's' : ''} • Real-time monitoring enabled
                  </p>
                </div>
              </div>
            </div>
