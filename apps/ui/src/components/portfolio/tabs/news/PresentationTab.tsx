...
import { Upload, Bot, FileText, FileSpreadsheet, Presentation, Zap, ArrowRight, Check, AlertCircle, Trash2, Download } from 'lucide-react';
...
const integrationOptions = [
  { id: 'contractor-profiles', name: 'Contractor Profiles', description: 'Company overviews, performance metrics, contact information' },
  { id: 'financial-data', name: 'Financial Data', description: 'Revenue history, award amounts, contract values' },
  { id: 'performance-metrics', name: 'Performance Metrics', description: 'Composite scores, efficiency ratings, reliability metrics' },
  { id: 'network-analysis', name: 'Network Analysis', description: 'Relationship maps, partnership data, subcontractor networks' },
  { id: 'market-intelligence', name: 'Market Intelligence', description: 'Competitive positioning, market share, industry benchmarks' },
  { id: 'compliance-data', name: 'Compliance Data', description: 'Certifications, regulatory status, audit results' },
];
  ...
  const handleStartIntegration = () => {
    ...
    setTimeout(() => {
      setIntegrationRequests(prev => prev.map(r =>
        r.id === newRequest.id
          ? { ...r, status: 'completed', result: 'Integration completed successfully! Your file has been enhanced with GoldenGate data.' }
          : r
      ));
    }, 5000);
  };
                  ...
                  <div className="relative h-full">
                    ...
                    <div className="pt-8">
                      <div className="space-y-4">
                        {/* Category Groups */}
                        {['Financial', 'Performance', 'Market', 'Relationships', 'Portfolio', 'Activity'].map(category => {
                          const categoryCharts = availableCharts.filter(chart => chart.category === category);
                          return (
                            <div key={category}>
                                ...
                                {categoryCharts.map(chart => (
                                  <div
                                    ...
                                    className={`p-3 border rounded-lg cursor-pointer transition-all ${
                                      selectedCharts.some(slide => slide.chartType === chart.id)
                                  ...
                                  >
                                    <div className="flex items-start gap-3">
                                      <div className={`p-2 rounded ${selectedCharts.some(slide => slide.chartType === chart.id) ? 'bg-[#8B8EFF]/20' : 'bg-gray-700/30'}`}>
                                        <chart.icon className={`w-4 h-4 ${selectedCharts.some(slide => slide.chartType === chart.id) ? 'text-[#8B8EFF]' : 'text-gray-400'}`} />
                                      </div>
                                      <div className="flex-1">
                                        <h6 className="text-sm font-medium text-white mb-1">{chart.name}</h6>
              ...
              <div className="flex-1">
                <ChartStyleContainer>
                  <div className="relative h-full">
                    ...
                    {/* Live Indicator */}
                    <div className="absolute top-0 right-0 z-10 flex items-center gap-2">
                      <span className="text-xs text-gray-400">{selectedCharts.length} selected</span>
                    </div>
                    ...
                    <div className="pt-8">
                      {selectedCharts.length === 0 ? (
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-400 text-sm">No charts selected</p>
                          <p className="text-gray-500 text-xs mt-1">Select charts from the library to export</p>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          {selectedCharts.map((slide, index) => {
                            const chart = availableCharts.find(c => c.id === slide.chartType);
                            if (!chart) return null;

                            return (
                              <div key={slide.id} className="p-3 bg-black/20 border border-gray-700/30 rounded-lg">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div>
                                        <h6 className="text-sm text-white font-medium">{chart.name}</h6>
                                        <p className="text-xs text-gray-400"><span className="font-bold">{slide.assetName.toUpperCase()}</span> • {slide.assetUei.toUpperCase()}</p>
                                      </div>
                                    </div>
                                  </div>
                                  <div className="flex items-center gap-1">
                                    ...
                                    <button
                                      onClick={() => removeSelectedChart(slide.id)}
            ...
            <div className="space-y-2">
              {availableAssets.map(asset => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400 uppercase">{asset.uei}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="h-screen flex items-center justify-center p-4">
            <div className="bg-gray-900 border border-[#8B8EFF]/30 rounded-xl w-full max-w-4xl h-[80vh] flex flex-col">
              <div className="flex items-center justify-between px-4 py-2 border-b border-gray-700/30 flex-shrink-0">
                <div>
                  <h3 className="text-base font-medium text-white">{previewSlide.title}</h3>
                  <p className="text-xs text-gray-400">
                    <span className="font-bold">{previewSlide.assetName.toUpperCase()}</span> • {previewSlide.assetUei.toUpperCase()}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    onClick={() => window.open(`/platform/contractor-detail/${previewSlide.assetUei}`, '_blank')}
              ...
              <div className="flex-1 p-2 overflow-auto">
                <div className="bg-black/20 border border-gray-700/30 rounded-lg h-full overflow-auto">
                  <ChartPreviewComponent chartType={previewSlide.chartType} assetData={previewSlide} />
                </div>
              </div>
            </div>
          </div>
