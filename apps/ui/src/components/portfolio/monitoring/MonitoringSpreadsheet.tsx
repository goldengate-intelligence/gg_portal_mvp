import { ArrowLeft, BarChart3, Database, Download } from "lucide-react";
			...
			switch (type) {
				case "performance": {
					const perfValues = [
						...
						`$${(
							(contractor.activeAwards / 1000000) * (0.8 + Math.random() * 0.4)
						).toFixed(1)}M`, // Awards Captured
						`$${(
							(contractor.activeAwards / 1000000) * (0.6 + Math.random() * 0.8)
						).toFixed(1)}M`, // Revenue
						`$${(
							(contractor.activeAwards / 1000000) * (1.5 + Math.random() * 2)
						).toFixed(1)}M`, // Pipeline
				...
				case "activity": {
					const activityValues = [
						Math.floor(
							(contractor.activeAwards / 5000000) * (0.8 + Math.random() * 0.4),
						), // New Awards
						Math.floor(
							(contractor.activeAwards / 8000000) * (0.6 + Math.random() * 0.8),
						), // Modifications
						Math.floor(
							(contractor.activeAwards / 6000000) * (0.7 + Math.random() * 0.6),
						), // Pipeline Activity
						Math.floor(
							(contractor.activeAwards / 10000000) * (0.5 + Math.random() * 1),
						), // Award Velocity
						Math.floor(
							(contractor.activeAwards / 12000000) *
								(0.4 + Math.random() * 1.2),
						), // Contract Extensions
						Math.floor(
							(contractor.activeAwards / 3000000) * (1.2 + Math.random() * 0.6),
						), // All Events
					];
								...
								<div className="border border-gray-700 rounded-lg bg-gray-800">
									...
									<div className="bg-gray-800 border-b border-gray-700/50 p-3">
										<div className="flex items-center justify-between">
											...
											<div className="text-xs text-gray-400">
												{data.length} contractors • Sorted by active awards
												value
											</div>
										</div>
									</div>
										...
										<table className="w-full text-sm">
											<thead className="bg-gray-900/50">
												<tr>
													...
													{columns.map((column, index) => (
														<th
															key={index}
															className={`text-center p-3 ${config.textColor} font-medium border-b border-gray-700/50 min-w-[120px]`}
														>
															{column}
														</th>
													))}
											...
											<tbody>
												{data.map((row, rowIndex) => (
													<tr
														key={rowIndex}
														className="hover:bg-gray-700/30 transition-colors border-b border-gray-800/50"
													>
														...
														{row.slice(1).map((cell, cellIndex) => (
															<td
																key={cellIndex}
																className="p-3 text-center text-gray-200"
															>
																{cell}
															</td>
														))}
									...
									<div className="bg-gray-900/50 border-t border-gray-700/50 p-3">
										<div className="flex items-center justify-between text-xs text-gray-400">
											<div>Query executed in 127ms</div>
											<div>
												Total portfolio value: $
												{data
													.reduce((sum, row) => sum + Math.random() * 100, 0)
													.toFixed(1)}
												M
											</div>
										</div>
