...
export default function Header() {
	...
	return (
		<header
			className="px-6 py-4 border-b border-gray-600/20 fixed top-0 left-0 right-0 z-50"
			style={{ backgroundColor: getHeaderBackground() }}
		>
			<nav className="flex items-baseline justify-between text-white">
				<div className="flex items-baseline">
					<div
						className="py-1 text-[#D2AC38] mr-6 text-2xl"
						style={{ fontFamily: "Michroma, sans-serif" }}
					>
						<Link
							to="/"
							className="hover:brightness-125 transition-all tracking-tighter"
						>
							Goldengate
						</Link>
					</div>
					...
					{isAuthenticated && (
						<>
							...
							>
								<Link
									to="/platform"
									className="text-gray-300 hover:brightness-125 transition-all header-nav-text"
								>
									Platform
								</Link>
							</div>
							<div
								className="px-3 py-1 text-lg ml-4"
								style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}
							>
								<Link
									to="/dashboard"
					...
					{isAuthenticated ? (
						<div className="flex items-baseline gap-4">
							...
							>
								<button
									...
									className="text-red-500 hover:brightness-125 transition-all cursor-pointer header-nav-text bg-transparent border-0 p-0"
								>
									Sign Out
								</button>
							</div>
						...
						<>
							...
							>
								<Link
									to="/register"
									className="text-[#D2AC38] hover:brightness-125 transition-all"
								>
									Register
								</Link>
							</div>
