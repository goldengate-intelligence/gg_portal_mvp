...
import { Database, BarChart3, Copy, Download, History } from 'lucide-react';
      ...
      <div className="bg-gray-800 mb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 px-4">
            <button className="p-2 text-gray-400 hover:text-gray-300 hover:bg-gray-700 rounded transition-colors" title="Copy">
            ...
            {queryResults.length > 0 ? (
                ...
                <thead className="sticky top-0 bg-gray-800 border-b border-gray-700">
                  <tr>
                    {queryColumns.map((column, index) => (
                      <th key={index} className="text-left p-3 text-gray-300 font-medium border-r border-gray-700 last:border-r-0">
                        <div className="flex items-center gap-2">
                          <span>{column}</span>
                          <div className="text-xs text-gray-500">
                            {column.includes('OBLIGATED') || column.includes('VALUE') ? 'NUMBER(38,0)' : 'VARCHAR(16777216)'}
                          </div>
                        </div>
                      </th>
                    ))}
                  ...
                  {queryResults.map((row, rowIndex) => (
                    ...
                    >
                      {queryColumns.map((column, colIndex) => (
                        <td key={colIndex} className="p-3 text-gray-200 border-r border-gray-700 last:border-r-0 font-mono text-xs">
                          {(row as any)[column] || (
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">{queryExecutionTime}s elapsed</div>
                </div>
              </div>
            ) : (
              <div className="h-full flex items-center justify-center">
