...
export const contractorApi = {
    ...
    if (params.filters) {
      if (params.filters.location) {
        filteredResults = filteredResults.filter(c =>
          c.location.toLowerCase().includes(params.filters!.location!.toLowerCase())
        );
      }
      if (params.filters.certification) {
        filteredResults = filteredResults.filter(c =>
          c.certification_level === params.filters!.certification
        );
      }
    }
  ...
  async exportResults(contractors: Contractor[], format: 'csv' | 'xlsx' = 'csv'): Promise<Blob> {
