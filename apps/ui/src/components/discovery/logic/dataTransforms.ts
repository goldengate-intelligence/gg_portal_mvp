import type { ContractorData, Contractor } from '../types/discovery';
...
export const generateMockContractors = (count: number = 25): Contractor[] => {
  const companies = [
    'Lockheed Martin Corporation', 'Boeing Company', 'Raytheon Technologies',
    'General Dynamics', 'Northrop Grumman', 'L3Harris Technologies',
    'BAE Systems', 'CACI International', 'SAIC', 'Booz Allen Hamilton',
    'Accenture Federal Services', 'IBM Federal', 'Microsoft Federal',
    'Amazon Web Services', 'Oracle America', 'Dell Technologies Federal'
  ];
  ...
  const locations = [
    'Arlington, VA', 'Bethesda, MD', 'McLean, VA', 'Reston, VA',
    'Alexandria, VA', 'Rockville, MD', 'Fairfax, VA', 'Tysons, VA'
  ];
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    naics_description: naicsCodes[Math.floor(Math.random() * naicsCodes.length)],
    location: locations[Math.floor(Math.random() * locations.length)],
    active_awards_value: `$${(Math.random() * 1000000000).toFixed(0)}`,
    performance_score: Math.floor(Math.random() * 40) + 60,
    certification_level: certifications[Math.floor(Math.random() * certifications.length)],
