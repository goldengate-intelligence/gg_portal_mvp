...
const PartnershipVelocityHeatmap: React.FC<PartnershipVelocityHeatmapProps> = ({
  ...
  const columns = [
    { key: 'concentration', label: 'Concentration', tooltip: 'Revenue concentration from this partner' },
    { key: 'freshness', label: 'Freshness', tooltip: 'Engagement velocity and recency' },
    { key: 'geoSynergy', label: 'Geo Synergy', tooltip: 'Geographic optimization score' },
    { key: 'dependency', label: 'Dependency', tooltip: 'Strategic dependency level' },
    { key: 'composite', label: 'Composite', tooltip: 'Overall partnership momentum score' }
  ];

  return (
    <div className="w-full h-full flex flex-col">
      <h3 className="text-2xl text-gray-200 font-normal tracking-wider mb-4" 
      ...
      <div className="flex-1 overflow-auto" style={{ maxHeight: height }}>
          ...
          {sortedData.slice(0, 10).map((partner, idx) => (
            <React.Fragment key={partner.partnerUei}>
              ...
              <div className="text-xs text-gray-300 p-2 flex items-center gap-2 bg-gray-900/40">
                <span className={`text-xs ${partner.relationshipType === 'prime' ? 'text-[#5BC0EB]' : 'text-[#FF4C4C]'}`}>
              ...
              {/* Score Cells */}
              {columns.map((col) => {
                const score = partner[col.key as keyof PartnershipData] as number;
                ...
                return (
                  <div key={col.key}
                    ...
                    {isComposite && (
                      <div className="absolute inset-0 ring-1 ring-[#D2AC38] opacity-50"></div>
                    )}
                  </div>
                );
              })}
        ...
        <div className="mt-4 flex items-center justify-between text-xs text-gray-400">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded" style={{ backgroundColor: 'rgb(128, 0, 128)' }}></div>
