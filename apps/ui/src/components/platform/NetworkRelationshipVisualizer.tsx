...
export function NetworkRelationshipVisualizer({
  ...
  const metrics = useMemo(() => {
    ...
    const primePartners = graphData.nodes.filter(n => n.type === 'prime').length;
    const subPartners = graphData.nodes.filter(n => n.type === 'sub').length;
    const totalRevenue = graphData.links.reduce((sum, link) => sum + link.revenue, 0);
    const avgStrength = graphData.links.length > 0
      ? graphData.links.reduce((sum, link) => sum + link.strength, 0) / graphData.links.length
      : 0;
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl text-white">Network Relationships</CardTitle>
      ...
      <CardContent className="p-0">
        ...
        <div className="grid grid-cols-5 gap-4 p-4 border-b border-gray-700">
          <div>
            ...
            <p className="text-lg font-semibold text-white">{metrics.totalPartners}</p>
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-lg font-semibold text-green-400">{metrics.primePartners}</p>
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-lg font-semibold text-blue-400">{metrics.subPartners}</p>
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
        ...
        <div className="flex items-center gap-4 p-4 border-b border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-32"
            />
            <span className="text-sm text-white font-sans">{filterThreshold}</span>
          </div>
        ...
        <div className="relative bg-black/50" style={{ height: '600px' }}>
          <ForceGraph2D
            ...
            nodeLabel="name"
            nodeColor={(node: any) => 
              highlightNodes.has(node.id) ? node.color : 
              hoverNode && !highlightNodes.has(node.id) ? 'rgba(75, 85, 99, 0.3)' : 
              node.color
            }
            nodeRelSize={1}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <Badge className={`text-xs ${
                    selectedNode.type === 'prime' ? 'bg-green-500/20 text-green-400' :
                    selectedNode.type === 'sub' ? 'bg-blue-500/20 text-blue-400' :
                    'bg-yellow-500/20 text-yellow-400'
                  }`}>
                    {selectedNode.type === 'prime' ? 'Prime Contractor' :
                     selectedNode.type === 'sub' ? 'Subcontractor' : 'Hybrid'}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-gray-300 font-sans">{selectedNode.id}</span>
                </div>
              </div>
            </div>
          )}
