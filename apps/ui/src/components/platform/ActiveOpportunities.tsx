...
import { MetricCard, OpportunityCard } from "../ui/card";
...
export function ActiveOpportunities() {
	...
	const [opportunityType, setOpportunityType] = useState<"all" | "AWD" | "IDV">(
		"all",
	);
	const [sortBy, setSortBy] = useState<"value" | "deadline" | "posted">(
		"deadline",
	);
		...
		results.sort((a, b) => {
			switch (sortBy) {
				...
				case "deadline":
					...
					return (
						new Date(a.responseDeadline).getTime() -
						new Date(b.responseDeadline).getTime()
					);
				case "posted":
					return (
	...
	const metrics = useMemo(() => {
		...
		const expiringCount = filteredOpportunities.filter((o) => {
			...
			const daysUntil = Math.ceil(
				(new Date(o.responseDeadline).getTime() - Date.now()) /
					(1000 * 60 * 60 * 24),
			);
			return daysUntil <= 7 && daysUntil > 0;
		}).length;

		const highRiskCount = filteredOpportunities.filter(
