...
import { Checkbox, CheckboxGroup, RadioGroup, Radio } from '../ui/checkbox';
import { ExponentialSlider, RangeSlider } from '../ui/slider';
import { Select, SelectTrigger, SelectContent, SelectItem, SelectValue } from '../ui/select';
import { Badge } from '../ui/badge';
import { INDUSTRIES, US_STATES, STATE_NAMES } from '../../data/industries';
import type { SearchFilters, IndustrySector, LocationType, LifecycleStage, BusinessMomentum, OwnershipType } from '../../types';
  ...
  return (
    <div className="h-full flex flex-col bg-dark-gray">
      ...
      <div className="flex items-center justify-between p-6 border-b border-dark-gray">
        <h2 className="text-lg font-semibold text-yellow-500 font-aptos">Filters</h2>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Location</h3>
          <CheckboxGroup
          ...
          {filters.location.includes('US') && (
            <div className="mt-3 space-y-2">
              <label className="text-xs text-gray-400 font-aptos">Select States</label>
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        ...
                        handleStateChange(filters.states.filter(s => s !== state));
                      }
                    }}
                ...
                <div className="flex flex-wrap gap-1">
                  {filters.states.map(state => (
                    ...
                    >
                      {state}
                      <button
                        onClick={() => handleStateChange(filters.states.filter(s => s !== state))}
        ...
        {/* Industry Sectors */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Industry Sectors</h3>
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  if (checked) {
                    ...
                    handleIndustryChange(filters.sectors.filter(s => s !== industry.id));
                  }
                }}
                label={industry.name}
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Contract Value Range</h3>
          <div className="space-y-4">
            <div className="flex justify-between text-xs text-gray-400">
              <span>{formatCurrency(filters.contractValueMin || 0)}</span>
              <span>{formatCurrency(filters.contractValueMax || 10000000000)}</span>
            </div>
            <ExponentialSlider
              value={filters.contractValueMin || 0}
              onValueChange={(value) => handleContractValueChange(value, filters.contractValueMax || 10000000000)}
              min={0}
              max={10000000000}
            />
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => handleContractValueChange(100000000, 10000000000)}
                className="text-xs"
              >
                &gt;$100M
              </Button>
        ...
        {/* Lifecycle Stage */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Lifecycle Stage</h3>
          <CheckboxGroup
        ...
        {/* Business Momentum */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Business Momentum</h3>
          <CheckboxGroup
            values={filters.businessMomentum}
            onValuesChange={handleMomentumChange}
            options={[
              { value: 'high-growth', label: 'High Growth', description: 'Rapid expansion' },
              { value: 'steady-growth', label: 'Steady Growth', description: 'Consistent growth' },
              { value: 'stable', label: 'Stable', description: 'Maintaining position' },
              { value: 'declining', label: 'Declining', description: 'Losing momentum' },
              { value: 'volatile', label: 'Volatile', description: 'Unpredictable' },
            ]}
          />
        </div>
        ...
        {/* Ownership Type */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Ownership Type</h3>
          <CheckboxGroup
        ...
        {/* Additional Filters */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-white font-aptos">Performance Score</h3>
