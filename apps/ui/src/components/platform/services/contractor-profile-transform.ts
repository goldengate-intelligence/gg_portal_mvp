 ...
 */
export function transformContractorProfile(
	profile: ContractorProfile,
): Contractor {
	const totalObligatedValue = Number.parseFloat(profile.totalObligated) || 0;
	...
	return {
		...
		industry: (industryMap[profile.primaryIndustryCluster || "Other"] ||
			"other") as any,
		...
		lifecycleStage: (lifecycleStageMap[
			profile.dominantLifecycleStage || "Active"
		] || "active") as any,
		businessMomentum: businessMomentum as any,
		ownershipType: (ownershipMap[profile.dominantSizeTier || "SMALL"] ||
			"private") as any,
 ...
 */
export function transformContractorProfileArray(
