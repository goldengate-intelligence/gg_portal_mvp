...
export function transformContractorData(apiData: ContractorData): Contractor {
	const totalObligatedValue = Number.parseFloat(apiData.totalObligated) || 0;
	...
	return {
		...
		industry: (industryMapping[apiData.industryCluster || "Other"] ||
			"other") as any,
		location: (apiData.country === "U.S."
			? "US"
			: apiData.country || "Unknown") as any,
		...
		lifecycleStage: (lifecycleMapping[apiData.lifecycleStage || "Active"] ||
			"active") as any,
		businessMomentum: (momentumMapping[apiData.lifecycleStage || "Active"] ||
			"steady-growth") as any,
		ownershipType: (ownershipMapping[apiData.sizeTier || "SMALL"] ||
			"private") as any,

		totalContractValue: totalObligatedValue,
		activeContracts: apiData.totalContracts,
		pastPerformanceScore: Math.max(
			80,
			Math.min(100, 85 + apiData.agencyDiversity * 2),
		), // Score based on agency diversity

		createdAt: new Date(apiData.cacheCreatedAt),
		updatedAt: new Date(apiData.cacheUpdatedAt),
		lastVerified: apiData.sourceLastUpdated
			? new Date(apiData.sourceLastUpdated)
			: new Date(apiData.cacheUpdatedAt),
 ...
 */
export function transformContractorDataArray(
	apiDataArray: ContractorData[],
): Contractor[] {
