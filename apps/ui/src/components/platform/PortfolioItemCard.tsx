...
export function PortfolioItemCard({
	...
	const getMomentumIcon = (momentum: string) => {
		if (momentum.includes("growth"))
			return <TrendingUp className="h-3 w-3 text-green-400" />;
		if (momentum.includes("declining"))
			...
			<div ref={setNodeRef} style={style}>
				<Card variant="interactive" className="relative group">
					...
					<CardHeader className="pb-3">
						<CardTitle size="md" className="pr-8">
							{item.contractor.name}
						</CardTitle>
						<CardDescription>
							<div className="flex items-center gap-2">
								<span className="font-sans text-xs">{item.contractor.uei}</span>
								{item.alertsEnabled && (
					...
					<CardContent className="space-y-3">
						<div className="flex flex-wrap gap-1">
							...
							{item.contractor.pastPerformanceScore && (
								<PerformanceBadge
									score={item.contractor.pastPerformanceScore}
									showScore={false}
								/>
							)}
						</div>
						...
						<div className="space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<span className="text-white font-medium">
									{item.contractor.totalContractValue
										? formatCurrency(item.contractor.totalContractValue)
										: "N/A"}
								</span>
							</div>
							<div className="flex justify-between">
