...
export function CompetitiveBenchmarkPanel({ 
  ...
  const radarData = [
    ...
    { metric: 'Performance', value: currentPercentiles.performance, fullMark: 100 },
    ...
    <Card className="bg-medium-gray border-yellow-500/20">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-yellow-500 font-aptos">Competitive Benchmarking</CardTitle>
            <CardDescription className="text-gray-400">
              Peer comparison across {peerGroup.size.toLocaleString()} similar contractors
            </CardDescription>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <MarketPositionIcon.icon className={`h-5 w-5 ${MarketPositionIcon.color}`} />
              <span className={`font-semibold ${MarketPositionIcon.color} capitalize`}>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-dark-gray rounded-lg p-4 border border-gray-700">
            ...
            <div className={`text-2xl font-bold ${getPercentileColor(currentPercentiles.revenue)}`}>
          ...
          <div className="bg-dark-gray rounded-lg p-4 border border-gray-700">
            ...
            <div className={`text-2xl font-bold ${getPercentileColor(currentPercentiles.growth)}`}>
          ...
          <div className="bg-dark-gray rounded-lg p-4 border border-gray-700">
            ...
            <div className={`text-2xl font-bold ${getPercentileColor(currentPercentiles.performance)}`}>
          ...
          <div className="bg-dark-gray rounded-lg p-4 border border-gray-700">
            ...
            <div className={`text-2xl font-bold ${getPercentileColor(currentPercentiles.overall)}`}>
        ...
        {data.historicalComparisons && data.historicalComparisons.length > 1 && (
          <div className="bg-dark-gray rounded-lg p-4 border border-gray-700">
            <h4 className="text-sm font-semibold text-gray-300 mb-3">Percentile Trends</h4>
            <div className="space-y-2">
              {data.historicalComparisons.slice(-3).map((comparison, idx) => (
                <div key={idx} className="flex items-center justify-between text-xs">
                  <span className="text-gray-400">
                    {new Date(comparison.monthYear).toLocaleDateString('en-US', { 
                      month: 'short', 
                      year: 'numeric' 
                    })}
                  </span>
                  <div className="flex gap-4">
                    <Badge variant={getPercentileBadgeVariant(comparison.revenuePercentile)}>
                      Rev: {comparison.revenuePercentile}
                    </Badge>
                    <Badge variant={getPercentileBadgeVariant(comparison.growthPercentile)}>
                      Grw: {comparison.growthPercentile}
                    </Badge>
                    <Badge variant={getPercentileBadgeVariant(comparison.overallPerformanceScore)}>
        ...
        <div className="bg-dark-gray rounded-lg p-4 border border-gray-700">
          <h4 className="text-sm font-semibold text-gray-300 mb-3">Competitive Insights</h4>
