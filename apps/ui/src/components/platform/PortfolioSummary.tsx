...
export function PortfolioSummary({
	...
	const industryDistribution = portfolio.contractors.reduce(
		(acc, item) => {
	...
	const momentumDistribution = portfolio.contractors.reduce(
		(acc, item) => {
	...
	const topPerformers = [...portfolio.contractors]
		.sort(
			(a, b) =>
				(b.contractor.pastPerformanceScore || 0) -
				(a.contractor.pastPerformanceScore || 0),
		)
		.slice(0, 3);

	// Get contractors needing attention (low performance or expiring)
	const needsAttention = portfolio.contractors.filter(
		(item) =>
			(item.contractor.pastPerformanceScore &&
				item.contractor.pastPerformanceScore < 80) ||
	...
	return (
		<div className="p-6 space-y-6">
			{/* Portfolio Overview */}
			<div>
				<h3 className="text-lg font-semibold text-yellow-500 font-aptos mb-4">
					Portfolio Overview
				</h3>
				<div className="space-y-3">
					...
					{portfolio.lastAnalyzed && (
						<div className="flex justify-between items-center">
							...
							<span className="text-sm text-white">
								{formatDate(portfolio.lastAnalyzed)}
							</span>
						</div>
					)}
				</div>
							...
							<div className="flex items-center gap-2">
								<div className="w-24 h-2 bg-gold-700 rounded-full overflow-hidden">
									<div
										...
										style={{
											width: `${(count / metrics.contractorCount) * 100}%`,
										}}
									/>
								</div>
			...
			<div>
				...
				<div className="space-y-2">
					{topPerformers.map((item, index) => (
						<div
							key={item.id}
							className="flex items-center justify-between p-2 bg-dark-gold rounded"
						>
							<div className="flex items-center gap-2">
				...
				<div className="space-y-2">
					{Object.entries(momentumDistribution).map(([momentum, count]) => (
						<div key={momentum} className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								{momentum.includes("growth") && (
									<TrendingUp className="h-4 w-4 text-green-400" />
								)}
								{momentum.includes("declining") && (
