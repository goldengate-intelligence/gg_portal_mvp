			...
			<div className={cn("relative w-full", className)} ref={ref} {...props}>
				{showValue && (
					<div className="flex justify-between mb-2">
						<span className="text-sm text-gray-400 font-aptos">
...
const RangeSlider = React.forwardRef<HTMLDivElement, RangeSliderProps>(
	(
		...
		const updateValue = (e: MouseEvent) => {
			...
			const newValues: [number, number] = [...internalValue] as [
				number,
				number,
			];
		...
		const displayValues = internalValue.map((v) =>
					...
					{[0, 1].map((index) => (
						<div
							...
							className={cn(
								...
								!disabled && "hover:shadow-lg hover:shadow-yellow-500/25",
								draggingIndex === index &&
									"shadow-lg shadow-yellow-500/25 z-10",
							)}
...
export const ExponentialSlider = React.forwardRef<
	HTMLDivElement,
	ExponentialSliderProps
>(
	(
		{
			value = 0,
