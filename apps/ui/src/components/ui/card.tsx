...
>(({ className, variant = "default", ...props }, ref) => {
	const variants = {
		...
		elevated:
			"bg-medium-gray border border-gray-700/30 shadow-lg shadow-black/25 text-white",
		outlined: "bg-transparent border border-gray-600/30 text-white",
		interactive:
			"bg-medium-gray border border-gray-700/30 hover:border-gray-600/50 hover:shadow-lg hover:shadow-gray-600/10 text-white transition-all duration-200 cursor-pointer",
	};
						...
						<span
							className={cn(
								...
								contractor.pastPerformanceScore &&
									contractor.pastPerformanceScore >= 90
									? "text-green-400"
									: contractor.pastPerformanceScore &&
											contractor.pastPerformanceScore >= 80
										? "text-yellow-400"
										: "text-red-400",
							)}
						>
							{contractor.pastPerformanceScore
								? `${contractor.pastPerformanceScore}/100`
								: "N/A"}
						</span>
					</div>
				</CardContent>
...
>(
	(
		{ className, title, value, description, trend, trendValue, ...props },
		ref,
	) => {
		const trendIcons = {
			up: "↗",
		...
		return (
			<Card ref={ref} className={cn("p-6", className)} {...props}>
				<div className="flex flex-row items-center justify-between space-y-0 pb-2">
					<CardTitle size="sm" className="text-gray-300">
						{title}
					</CardTitle>
					{trend && (
				...
				<div>
					<div className="text-2xl font-bold text-yellow-500 font-aptos">
						{value}
					</div>
					{description && (
