...
const MultiSelect = React.forwardRef<HTMLDivElement, MultiSelectProps>(
	(
		{ values, onValuesChange, disabled, className, children, ...props },
		ref,
	) => {
		const [isOpen, setIsOpen] = React.useState(false);
		const selectRef = React.useRef<HTMLDivElement>(null);
		...
		return (
			<MultiSelectContext.Provider
				value={{ values, onValuesChange, isOpen, setIsOpen, disabled }}
			>
				<div ref={selectRef} className={cn("relative", className)} {...props}>
					{children}
				</div>
...
>(({ children, className, ...props }, ref) => {
	...
	return (
		<button
			...
			{...props}
		>
			<div className="flex flex-wrap gap-1 flex-1 min-h-[20px]">{children}</div>
			<ChevronDown
				className={cn(
