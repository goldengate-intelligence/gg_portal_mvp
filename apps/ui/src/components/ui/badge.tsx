	...
	"inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 font-aptos",
	{
		variants: {
			variant: {
				...
				secondary:
					"border-transparent bg-medium-gray text-gray-300 hover:bg-lighter-gray",
				destructive:
				...
				warning:
					"border-transparent bg-orange-500 text-black hover:bg-orange-600",
				...
				construction:
					"border-transparent bg-orange-900 text-orange-100 hover:bg-orange-800",
				healthcare:
					"border-transparent bg-pink-900 text-pink-100 hover:bg-pink-800",
				// Risk level variants
...
>(({ className, industry, ...props }, ref) => {
	const getIndustryVariant = (industry: string) => {
		...
		if (industry.includes("information-technology") || industry.includes("it"))
			return "it";
