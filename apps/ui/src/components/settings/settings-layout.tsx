...
import { ArrowLeft, User, Shield, Bot, BarChart3, Bell, HelpCircle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen text-white pb-20 bg-gradient-to-b from-black/90 via-gray-900/50 to-black/90 relative">
      ...
      <div className="container mx-auto px-6 pt-24 pb-20 relative z-10">
        ...
        <div className="mb-8">
          <Link to="/dashboard" className="inline-flex items-center gap-2 text-[#D2AC38] hover:text-[#D2AC38]/80 transition-colors mb-6">
            <ArrowLeft className="w-4 h-4" />
            <span className="text-sm font-medium" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
              Back to Dashboard
            </span>
          </Link>

          <div className="mb-8">
            <h1 className="text-4xl text-white mb-2" style={{ fontFamily: 'system-ui, -apple-system, sans-serif', fontWeight: '250' }}>
              {title}
            </h1>
          ...
          <div className="lg:col-span-1">
            <div className="border border-gray-700/50 hover:border-gray-600/40 rounded-xl overflow-hidden shadow-2xl transition-all duration-500 relative" style={{ backgroundColor: '#111726' }}>
              ...
              <div className="p-6 relative z-10">
                <h3 className="text-gray-200 mb-6 uppercase tracking-wide font-bold" style={{ fontFamily: 'Genos, sans-serif', fontSize: '18px' }}>
                  SETTINGS
                </h3>
                <nav className="space-y-2">
                  {settingsNavigation.map((item) => (
                    <Link
                      ...
                      activeProps={{
                        className: "bg-[#D2AC38]/20 border-[#D2AC38]/50 text-[#D2AC38]"
                      }}
                    >
                      <item.icon className="w-5 h-5 text-gray-400 group-hover:text-[#D2AC38] transition-colors" />
                      <span className="text-sm font-medium text-gray-300 group-hover:text-[#D2AC38] transition-colors" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
                        {item.name}
                      </span>
                    </Link>
                  ))}
