...
import { X, ExternalLink, Target, Database } from 'lucide-react';
...
export function HudContractorModal({
  ...
  return (
    <Modal open={isOpen} onOpenChange={onClose}>
      <ModalContent size="4xl" className="bg-black/95 border-2 border-cyan-500/50 shadow-[0_0_50px_rgba(0,217,255,0.3)] max-h-[90vh] overflow-hidden">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-green-400 font-sans uppercase">ACTIVE</span>
                    </div>
                    <h2 className="text-xl font-orbitron font-bold text-yellow-400 uppercase tracking-wide mb-1">
                      {contractor.name}
                    </h2>
                    {contractor.dbaName && (
                      <p className="text-sm text-cyan-400/70 font-sans">ALIAS: {contractor.dbaName}</p>
                    )}

                    {/* Tactical badges */}
                    <div className="flex items-center gap-2 mt-2">
                      <div className="px-2 py-1 bg-cyan-500/10 border border-cyan-500/30 rounded">
                        <span className="text-xs text-cyan-400 font-sans">UEI: {contractor.uei}</span>
                      </div>
              ...
              <div className="mt-4 pt-4 border-t border-cyan-500/10">
                <div className="grid grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-xs text-gray-400 font-sans uppercase mb-1">Contract Value</div>
                    <div className="text-lg font-bold text-cyan-400 font-sans">
                      {contractor.totalContractValue ? formatCurrency(contractor.totalContractValue) : 'CLASSIFIED'}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-gray-400 font-sans uppercase mb-1">Performance</div>
                    <div className="text-lg font-bold text-yellow-400 font-sans">
                      {contractor.pastPerformanceScore ? `${contractor.pastPerformanceScore}%` : 'N/A'}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-gray-400 font-sans uppercase mb-1">Active Ops</div>
                    <div className="text-lg font-bold text-green-400 font-sans">
                      {contractor.activeContracts?.toLocaleString() || 'N/A'}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-gray-400 font-sans uppercase mb-1">Agencies</div>
                    <div className="text-lg font-bold text-cyan-400 font-sans">
