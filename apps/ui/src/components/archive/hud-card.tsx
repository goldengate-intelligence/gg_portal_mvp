		...
		<div
			...
			style={{
				backgroundColor: !isPanel
					? CONTRACTOR_DETAIL_COLORS.backgroundColor
					: undefined,
			}}
		>
	...
	return (
		<div
			className={cn(
				...
				"border border-gray-700/30 rounded-lg backdrop-blur-sm",
				alert &&
					"border-red-500/30 bg-gradient-to-r from-red-500/10 to-transparent animate-pulse",
			)}
		...
		>
			<span
				className={cn(
					currentSize.label,
					"text-gray-400 uppercase tracking-wider font-normal",
				)}
			...
			<div className="flex items-center gap-2">
				...
				{unit && (
					<span className={cn(currentSize.unit, "text-gray-500 font-sans")}>
						{unit}
					</span>
				)}
				{trend && (
					<span
						className={cn(
							currentSize.trend,
							growth ? "text-green-400" : trendColors[trend],
						)}
					>
						{trendIcons[trend]}
					</span>
