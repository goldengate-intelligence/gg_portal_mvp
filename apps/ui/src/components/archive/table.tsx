...
>(({ className, ...props }, ref) => (
	<thead
		ref={ref}
		className={cn(
			"[&_tr]:border-b border-cyan-500/30 bg-black/70 backdrop-blur-sm",
			className,
		)}
		{...props}
	/>
...
>(({ className, children, sortable, sortDirection, onSort, ...props }, ref) => (
	<th
		...
		className={cn(
			...
			sortable &&
				"cursor-pointer hover:text-yellow-300 select-none transition-all duration-200 hover:shadow-[0_2px_10px_rgba(255,215,0,0.2)]",
			className,
		)}
...
>(({ className, ...props }, ref) => (
	<td
		ref={ref}
		className={cn(
			"px-6 py-4 align-middle text-cyan-400/90 font-sans text-sm [&:has([role=checkbox])]:pr-0 transition-all duration-200 group-hover:text-cyan-300",
			className,
		)}
		{...props}
	/>
	...
	if (items.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center p-12 text-cyan-400/70 font-orbitron bg-black/70 border-2 border-cyan-500/30 rounded-lg backdrop-blur-xl">
				...
				<div className="text-sm text-cyan-400/60 font-sans uppercase tracking-wide">
...
export function ContractorTable({
	...
	const columns = [
		...
		},
		{
			...
			render: (value: string) => (
				<div className="px-2 py-1 bg-black/50 border border-cyan-500/30 rounded font-sans text-xs text-yellow-400">
					{value}
				</div>
			),
		},
		{
			...
			sortable: true,
			render: (value: string) => (
				<span className="uppercase font-sans text-cyan-400/90">
					{value.replace("-", " ")}
				</span>
			),
		},
		{
			...
			render: (_: any, item: any) => (
				<div className="flex items-center gap-2">
					...
					<span className="font-sans text-cyan-400/90 uppercase">
		...
		},
		{
			...
			render: (value: number) =>
				value ? (
					...
					<span className="text-gray-500 font-sans">CLASSIFIED</span>
				),
		...
		},
		{
			...
			render: (value: number) => {
				if (!value)
					return (
						<div className="flex items-center justify-center">
							<span className="px-2 py-1 bg-gray-500/20 border border-gray-500/30 rounded text-gray-400 font-sans text-xs">
								UNKNOWN
							</span>
						</div>
					);
				...
				return (
					<div className="flex items-center justify-center">
						<div
							className={`px-2 py-1 border rounded font-sans text-xs font-bold ${color} flex items-center gap-1`}
						>
				...
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2 text-xs text-gray-400 font-sans">
						...
						<span className="text-yellow-400 font-bold">
							{contractors.length.toLocaleString()}
						</span>
					</div>
				</div>
			</div>
	...
	const columns = [
		...
		},
		{
			...
			render: (value: string) => {
				...
				return (
					<span
						className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${colors[value as keyof typeof colors]}`}
					>
