...
export function RelationshipCard({
	...
	const { awardedAmount, obligationRate, obligatedAmount } =
		calculateRelationshipMetrics(relationship, type);
	...
	return (
		...
		>
			{/* Header Section */}
			<div
				className={`relative h-28 ${isExpanded ? "rounded-t-xl" : "rounded-xl"}`}
			>
				<div
							...
							<div className="uppercase tracking-wide">
								...
								<div className="flex items-center justify-between">
									<div className="font-normal text-[#F97316]/90 text-xs">
										{relationship.contracts.length} Contract
										{relationship.contracts.length !== 1 ? "s" : ""} •{" "}
										{getWorkSummary(relationship)}
									</div>
									<div
				...
				<div className="flex items-center px-3 py-1 bg-gray-600/20 border border-gray-600/40 rounded-full gap-2 transition-all duration-200 hover:bg-gray-600/30 hover:border-gray-600/60">
					...
					<div
						...
						onClick={(e) => {
							...
							e.preventDefault();
							onSetActiveTooltip(null);
							onOpenAwardCardView(relationship, type, e);
						}}
					>
						<svg
							...
							onMouseEnter={(e) => {
								...
								e.currentTarget.style.filter =
									"drop-shadow(0 0 16px rgba(249, 115, 22, 1))";
