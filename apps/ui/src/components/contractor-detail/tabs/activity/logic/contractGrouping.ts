import type { Contract, Relationship, SortedInflowRelationships } from '../types';

export const groupByRelationship = (contractList: Contract[]): Relationship[] => {
...
export const sortInflowRelationships = (inflowRelationships: Relationship[]): SortedInflowRelationships => {
  const agencies = inflowRelationships.filter(r => r.type === 'agency').sort((a, b) => b.totalValue - a.totalValue);
  const primes = inflowRelationships.filter(r => r.type === 'prime').sort((a, b) => b.totalValue - a.totalValue);
