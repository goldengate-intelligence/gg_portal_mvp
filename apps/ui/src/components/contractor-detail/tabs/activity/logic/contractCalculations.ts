...
export const calculateRelationshipMetrics = (
	relationship: Relationship,
	type: "inflow" | "outflow",
) => {
	const awardedAmount =
		relationship.totalValue * (type === "inflow" ? 0.85 : 0.75);
	const obligationRate =
		relationship.contracts.reduce(
			(sum: number, contract: Contract) => sum + contract.utilization,
			0,
		) / relationship.contracts.length;
...
export const calculateContractTimeline = (contract: Contract) => {
	...
	const remainingMonths = Math.max(
		0,
		Math.floor(remainingTime / (1000 * 60 * 60 * 24 * 30)),
	);
