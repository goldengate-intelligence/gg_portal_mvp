...
export const ZIP_COORDINATES: Record<string, Coordinates> = {
  ...
  '22202': { lat: 38.8606, lng: -77.0500 }, // Arlington VA
  '22101': { lat: 38.8462, lng: -77.2470 }, // McLean VA
  '20814': { lat: 39.0458, lng: -77.0910 }, // Bethesda MD
...
export function mapPercentageToCoordinates(
  ...
  const lat = mapBounds.south + (mapBounds.north - mapBounds.south) * (1 - topPercent / 100);
  const lng = mapBounds.west + (mapBounds.east - mapBounds.west) * (leftPercent / 100);
...
export function coordinatesToMapPercentage(
  ...
  const leftPercent = ((coordinates.lng - mapBounds.west) / (mapBounds.east - mapBounds.west)) * 100;
  const topPercent = (1 - (coordinates.lat - mapBounds.south) / (mapBounds.north - mapBounds.south)) * 100;
...
export function getContractorLocations(contractors: any[]): LocationData[] {
  return contractors
    .map(contractor => getLocationCoordinates(
      contractor.zipCode,
      contractor.city,
      contractor.state
    ))
