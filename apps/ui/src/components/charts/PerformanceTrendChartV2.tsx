  ...
  const growthChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        label: 'YoY Growth %',
        data: growthData,
        borderColor: growthData[growthData.length - 1] >= 0 ? '#00FF88' : '#FF0066',
        backgroundColor: growthData[growthData.length - 1] >= 0 
          ? 'rgba(0, 255, 136, 0.1)' 
          : 'rgba(255, 0, 102, 0.1)'
      }]
    };
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-dark-gray p-3 rounded">
            ...
            <p className={`text-lg font-bold ${
              summary.growthTrend > 0 ? 'text-green-400' : 
              summary.growthTrend < 0 ? 'text-red-400' : 'text-gray-400'
            }`}>
              {summary.growthTrend > 0 ? '+' : ''}{summary.growthTrend?.toFixed(1) || 0}%
            </p>
          </div>
        </div>
