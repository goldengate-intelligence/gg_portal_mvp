...
export function AgencyExposureChart({
  ...
  const getRiskLevel = () => {
    if (concentrationRisk > 50) return { level: 'high', color: 'text-red-500', badge: 'destructive' };
    if (concentrationRisk > 35) return { level: 'medium', color: 'text-yellow-500', badge: 'warning' };
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-3">
          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between gap-4">
              ...
              <span className="font-medium text-white">{formatCurrency(data.value)}</span>
            </div>
            <div className="flex justify-between gap-4">
            ...
            <PieChart>
              ...
              <Legend 
                ...
                formatter={(value: string) => {
                  ...
                  const percentage = item ? ((item.value / totalValue) * 100).toFixed(1) : '0';
        ...
        <div className="mt-6 space-y-3">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 bg-gray-800 rounded-lg">
              ...
              <p className="text-sm font-semibold text-white">{topAgency.name}</p>
              <p className="text-xs text-gray-400 mt-1">
                {formatCurrency(topAgency.value)} ({concentrationRisk.toFixed(1)}%)
              </p>
            </div>
            <div className="p-3 bg-gray-800 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
          ...
          <div className="space-y-2">
            <p className="text-xs text-gray-400 uppercase tracking-wide">Top 3 Agencies</p>
            {data.slice(0, 3).map((agency, index) => {
              const percentage = ((agency.value / totalValue) * 100).toFixed(1);
              return (
                <div key={agency.name} className="flex items-center justify-between p-2 bg-gray-800/50 rounded">
                  <div className="flex items-center gap-2">
                    <div 
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">{agency.contracts} contracts</span>
