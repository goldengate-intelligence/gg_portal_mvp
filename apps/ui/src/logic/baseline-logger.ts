...
export class BaselineLogger {
	...
	logBaseline(
		...
		const log: BaselineLog = {
			...
			metadata: {
				...
				description:
					options.description || this.generateDefaultDescription(baseline),
				tags: options.tags || [],
	 ...
	 */
	private createDetailedSnapshot(
		baseline: ActivityBaseline,
	): Record<string, any> {
		...
		switch (baseline.feature) {
			...
			case "new_relationships":
				snapshot.breakdown = this.createRelationshipsBreakdown(
					baseline.details,
				);
				break;
		}
	...
	private createAwardsBreakdown(details: ActivityDetails): Record<string, any> {
		...
		details.sources.forEach((source) => {
			...
			breakdown.byContractType[contractType] =
				(breakdown.byContractType[contractType] || 0) + 1;
		...
		breakdown.averageValue =
			breakdown.totalValue / (details.sources.length || 1);
	 ...
	 */
	private createObligationsBreakdown(
		details: ActivityDetails,
	): Record<string, any> {
		const breakdown = {
		...
		details.sources.forEach((source) => {
			...
			breakdown.byType[obligationType] =
				(breakdown.byType[obligationType] || 0) + 1;

			const fiscalYear = metadata.fiscalYear?.toString() || "Unknown";
			breakdown.byFiscalYear[fiscalYear] =
				(breakdown.byFiscalYear[fiscalYear] || 0) + 1;

			breakdown.totalAmount += metadata.amount || 0;
		});

		breakdown.averageAmount =
			breakdown.totalAmount / (details.sources.length || 1);
	 ...
	 */
	private createSubawardsBreakdown(
		details: ActivityDetails,
	): Record<string, any> {
		const breakdown = {
		...
		details.sources.forEach((source) => {
			...
			breakdown.bySubcontractorType[subcontractorType] =
				(breakdown.bySubcontractorType[subcontractorType] || 0) + 1;

			const workType = metadata.workType || "Unknown";
			breakdown.byWorkType[workType] =
				(breakdown.byWorkType[workType] || 0) + 1;
	 ...
	 */
	private createRelationshipsBreakdown(
		details: ActivityDetails,
	): Record<string, any> {
		const breakdown = {
		...
		details.sources.forEach((source) => {
			...
			breakdown.byRelationshipType[relationshipType] =
				(breakdown.byRelationshipType[relationshipType] || 0) + 1;

			const partnerSize = metadata.partnerSize || "Unknown";
			breakdown.byPartnerSize[partnerSize] =
				(breakdown.byPartnerSize[partnerSize] || 0) + 1;

			const industry = metadata.industry || "Unknown";
			breakdown.byIndustry[industry] =
				(breakdown.byIndustry[industry] || 0) + 1;
		});
	...
	private generateDefaultDescription(baseline: ActivityBaseline): string {
		...
		const formattedValue = activityMonitoringService.formatActivityValue(
			baseline.feature,
	...
	updateLogMetadata(
		logId: string,
		metadata: Partial<BaselineLog["metadata"]>,
	): boolean {
	...
	generateStatelogSummary(logId: string): string {
		...
		const featureName =
			featureNames[log.feature as keyof typeof featureNames] || log.feature;
		...
		switch (log.feature) {
			case "new_awards":
				...
				if (breakdown.byContractType) {
					summary += `• By Type: ${Object.entries(breakdown.byContractType)
						.map(([type, count]) => `${type} (${count})`)
						.join(", ")}\n`;
				}
				if (breakdown.byAgency) {
					summary += `• By Agency: ${Object.entries(breakdown.byAgency)
			...
			case "new_subawards":
				...
				if (breakdown.bySubcontractorType) {
					summary += `• By Contractor Type: ${Object.entries(
						breakdown.bySubcontractorType,
					)
						.map(([type, count]) => `${type} (${count})`)
						.join(", ")}\n`;
				}
				if (breakdown.byWorkType) {
					summary += `• By Work Type: ${Object.entries(breakdown.byWorkType)
						.map(([type, count]) => `${type} (${count})`)
						.join(", ")}\n`;
				}
				break;
			...
			case "new_relationships":
				...
				if (breakdown.byRelationshipType) {
					summary += `• By Type: ${Object.entries(breakdown.byRelationshipType)
						.map(([type, count]) => `${type} (${count})`)
						.join(", ")}\n`;
				}
				if (breakdown.byIndustry) {
					summary += `• By Industry: ${Object.entries(breakdown.byIndustry)
