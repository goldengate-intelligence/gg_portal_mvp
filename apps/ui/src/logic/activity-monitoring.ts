...
export interface ActivityBaseline {
  ...
  entityId: string;
  entityName: string;
  feature: 'new_awards' | 'new_obligations' | 'new_subawards' | 'new_relationships';
...
export class ActivityMonitoringService {
  ...
  createBaseline(
    ...
    const currentData = this.getCurrentActivityData(entityId, feature, initialData);
  ...
  updateActivityValue(baselineId: string, newData?: any): ActivityDelta | null {
    ...
    // Get current state
    const currentData = this.getCurrentActivityData(baseline.entityId, baseline.feature, newData);
    ...
    const delta: ActivityDelta = {
      ...
      changeDate: new Date(),
      changeDetails: this.calculateChangeDetails(baseline.details, currentData.details),
  ...
  private getCurrentActivityData(
    ...
    if (mockData) {
      return {
        ...
        details: mockData.details || this.generateMockDetails(feature, mockData.value || 0)
      };
    }
   ...
   */
  private generateMockSourceName(feature: ActivityBaseline['feature'], index: number): string {
    const nameMap = {
   ...
   */
  private generateMockMetadata(feature: ActivityBaseline['feature']): Record<string, any> {
    const baseMetadata = {
      new_awards: {
        contractType: ['Prime', 'Subcontract', 'IDIQ'][Math.floor(Math.random() * 3)],
        agency: ['DoD', 'NASA', 'DOE', 'Commerce'][Math.floor(Math.random() * 4)],
        value: Math.floor(Math.random() * 1000000) + 100000
      },
      new_obligations: {
        obligationType: ['Initial', 'Additional', 'Modification'][Math.floor(Math.random() * 3)],
        amount: Math.floor(Math.random() * 500000) + 50000,
        fiscalYear: new Date().getFullYear()
      },
      new_subawards: {
        subcontractorType: ['Small Business', 'Large Business', '8(a)', 'WOSB'][Math.floor(Math.random() * 4)],
        workType: ['Manufacturing', 'Services', 'R&D', 'Support'][Math.floor(Math.random() * 4)],
        value: Math.floor(Math.random() * 200000) + 25000
      },
      new_relationships: {
        relationshipType: ['JV', 'Teaming', 'Supplier', 'Strategic'][Math.floor(Math.random() * 4)],
        partnerSize: ['Small', 'Medium', 'Large'][Math.floor(Math.random() * 3)],
        industry: ['Defense', 'Aerospace', 'Technology', 'Manufacturing'][Math.floor(Math.random() * 4)]
      }
    };
   ...
   */
  getRecentAlerts(hours: number = 24): ActivityDelta[] {
    const cutoff = new Date(Date.now() - hours * 60 * 60 * 1000);
    const allDeltas: ActivityDelta[] = [];

    this.deltas.forEach(deltas => {
      allDeltas.push(...deltas.filter(d =>
        d.exceedsThreshold && d.changeDate > cutoff
      ));
    });

    return allDeltas.sort((a, b) => b.changeDate.getTime() - a.changeDate.getTime());
  }
   ...
   */
  formatActivityValue(feature: ActivityBaseline['feature'], value: number): string {
   ...
   */
  private generateMockDetails(feature: ActivityBaseline['feature'], value: number): ActivityDetails {
