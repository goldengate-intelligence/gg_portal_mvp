/**
 * PERFORMANCE SUMMARY PANEL BACKUP V2
 * Saved: 2024-12-17
 *
 * This backup contains the updated PERFORMANCE SUMMARY panel from contractor-detail.tsx with:
 * - Composite score updated to 80 (average of 5 subscores)
 * - Performance tier changed from "Elite" to "Strong"
 * - Color changed from green (#38E54D) to teal (#4EC9B0) for 75-89 range
 * - Custom tooltips for all subscores with detailed explanations
 * - AI Analysis section (renamed from Performance Analysis)
 * - Green "TRACKING" indicator with pulse animation
 * - Color-coded subscores based on thresholds
 */
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 gap-8 mb-6 relative">
        ...
        <div className="border border-gray-600/20 rounded-xl backdrop-blur-sm p-4 relative" style={{ backgroundColor: CONTRACTOR_DETAIL_COLORS.containerColor }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-start gap-6">
              ...
              {/* Subscores with custom tooltips */}
                ...
                <div className="bg-black/60 border border-gray-700/30 rounded-md p-2 relative group/subscore">
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 invisible group-hover/subscore:visible opacity-0 group-hover/subscore:opacity-100 transition-opacity duration-200 z-50 pointer-events-none">
                    <div className="bg-black border border-gray-700 rounded-md px-3 py-2 text-xs text-gray-300 whitespace-nowrap">
                      ...
                      <div>Average number of years remaining in the</div>
                      <div>portfolio across all active awards.</div>
                    </div>
                    <div className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-l-4 border-l-transparent border-r-4 border-r-transparent border-t-4 border-t-gray-700"></div>
                  </div>
                </div>
                ...
                <div className="bg-black/60 border border-gray-700/30 rounded-md p-2 relative group/subscore">
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 invisible group-hover/subscore:visible opacity-0 group-hover/subscore:opacity-100 transition-opacity duration-200 z-50 pointer-events-none">
                    <div className="bg-black border border-gray-700 rounded-md px-3 py-2 text-xs text-gray-300 whitespace-nowrap">
                      ...
                      <div>Growth metrics normalized across awards</div>
                      <div>captured, revenue, and backlog expansion.</div>
                    </div>
                    <div className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-l-4 border-l-transparent border-r-4 border-r-transparent border-t-4 border-t-gray-700"></div>
                  </div>
                </div>
        ...
        <div className="border border-gray-600/20 rounded-xl backdrop-blur-sm p-4" style={{ backgroundColor: CONTRACTOR_DETAIL_COLORS.containerColor }}>
          <h4 className="font-sans text-xs uppercase tracking-wider mb-4 text-gray-500">
            PEER GROUP DETAILS
          </h4>
 ...
 * KEY UPDATES IN V2:
 *
 * 1. COMPOSITE SCORE CHANGES:
 *    - Updated from 92 to 80 (average of subscores: 82+76+91+68+85 / 5 = 80.4)
 *    - Color changed from #38E54D (green) to #4EC9B0 (teal) for 75-89 range
 *    - Stroke dasharray updated from 0.92 to 0.80 (80% completion)
 *    - Percentile text updated from "92nd" to "80th"
 *
 * 2. PERFORMANCE TIER:
 *    - Changed from "Elite" (90+) to "Strong" (75-89)
 *    - Color updated to match teal (#4EC9B0) for consistency
 *    - Updated in both header badge and Peer Group Details
 *
 * 3. AI ANALYSIS:
 *    - Renamed from "Performance Analysis" to "AI Analysis"
 *    - Updated text to reference score of 80 instead of 92
 *    - Changed from "elite performance" to "strong performance"
 *    - Changed from "top tier" to "high performance tier"
 *
 ...
 *
 * 5. SUBSCORE BAR COLORS:
 *    - Awards Captured (82): #4EC9B0 (teal - Strong range)
 *    - Revenue (76): #4EC9B0 (teal - Strong range)
 *    - Pipeline (91): #38E54D (green - Elite range)
 *    - Portfolio Duration (68): #FFD166 (amber - Stable range)
 *    - Blended Growth (85): #4EC9B0 (teal - Strong range)
 */