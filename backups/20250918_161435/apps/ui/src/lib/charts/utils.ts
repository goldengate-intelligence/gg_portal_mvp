 ...
 */
export function mergeChartOptions(...options: (Partial<ChartOptions> | undefined)[]): ChartOptions {
  const result: any = {};
 ...
 */
export function generateColors(count: number, scheme: 'primary' | 'gradient' | 'transparent' = 'gradient'): string[] {
  const schemes = {
 ...
 */
export function createAnimatedDataset(baseConfig: any, delay: number = 0): any {
  return {
    ...baseConfig,
    animation: {
      duration: 1000,
 ...
 */
export function getResponsiveOptions(baseOptions: ChartOptions, screenWidth: number): ChartOptions {
