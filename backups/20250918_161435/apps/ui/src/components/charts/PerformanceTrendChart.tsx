...
export function PerformanceTrendChart({
  ...
  const chartData = useMemo(() => {
    return data.map(point => ({
      ...
      revenue: parseFloat(point.monthlyRevenue?.toString() || '0'),
      ...
      pipeline: parseFloat(point.activePipelineValue?.toString() || '0')
    }));
  }, [data]);
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-3">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center justify-between gap-4 text-xs">
              ...
              <span className="font-medium text-green-500">{dataPoint.contracts}</span>
            </div>
            <div className="flex items-center justify-between gap-4 text-xs">
              <span className="text-gray-400">YoY Growth:</span>
              <span className={`font-medium ${dataPoint.growthYoY > 0 ? 'text-green-400' : 'text-red-400'}`}>
                {dataPoint.growthYoY > 0 ? '+' : ''}{dataPoint.growthYoY.toFixed(1)}%
              </span>
            </div>
            <div className="flex items-center justify-between gap-4 text-xs">
  ...
  const ActivityBadge = ({ status }: { status: string }) => {
    ...
    return (
      <Badge className={`${colors[status] || colors.unknown} border capitalize`}>
    ...
    <Card className="bg-medium-gray border-yellow-500/20">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-yellow-500 font-aptos">{title}</CardTitle>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              {getTrendIcon()}
              <Badge 
                variant={latestGrowth > 0 ? 'success' : latestGrowth < 0 ? 'destructive' : 'secondary'}
                className="font-mono"
              >
                {latestGrowth > 0 ? '+' : ''}{latestGrowth.toFixed(1)}% YoY
              </Badge>
            </div>
          </div>
        </div>
